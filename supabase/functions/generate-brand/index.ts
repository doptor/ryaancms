...
serve(async (req) => {
  ...
  try {
    ...
    // Step 1: Generate brand name using AI
    const nameResponse = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "system",
            content: "You are a brand naming expert. Given a project description, generate exactly ONE creative, catchy, memorable brand name. Rules: 1) Must be a single word, 2) 4-8 characters, 3) Easy to pronounce, 4) Modern and tech-friendly, 5) No existing major brand names. Respond with ONLY the brand name, nothing else."
          },
    ...
    // Step 2: Generate logo using AI image generation
    ...
    // Step 3: Update project with brand name and logo
    if (projectId) {
      try {
        const supabaseUrl = Deno.env.get("SUPABASE_URL")!;
        const supabaseKey = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!;
