...
export default function VendorListPage() {
  ...
  const { data: vendors, isLoading } = useQuery({
    queryKey: ["ac_vendors"],
    queryFn: async () => {
      const { data, error } = await supabase.from("ac_vendors").select("*").order("name");
  ...
  const upsert = useMutation({
    mutationFn: async () => {
      ...
      if (editId) {
        const { error } = await supabase.from("ac_vendors").update(payload).eq("id", editId);
        if (error) throw error;
      } else {
        const { error } = await supabase.from("ac_vendors").insert(payload);
        if (error) throw error;
      }
    },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["ac_vendors"] }); toast({ title: editId ? "Vendor updated" : "Vendor created" }); closeDialog(); },
    onError: (e: any) => toast({ title: "Error", description: e.message, variant: "destructive" }),
  });

  const deleteMut = useMutation({
    mutationFn: async (id: string) => { const { error } = await supabase.from("ac_vendors").delete().eq("id", id); if (error) throw error; },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["ac_vendors"] }); toast({ title: "Vendor deleted" }); },
  });
  ...
  const filtered = (vendors ?? []).filter(r => { const q = search.toLowerCase(); return !q || r.name.toLowerCase().includes(q) || (r.email || "").toLowerCase().includes(q); });
    ...
    <DashboardLayout>
      <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h1 className="text-2xl font-bold flex items-center gap-2"><Truck className="w-6 h-6 text-primary" /> Vendors</h1>
          ...
          <Dialog open={open} onOpenChange={v => { if (!v) closeDialog(); else setOpen(true); }}>
            ...
            <DialogContent>
              ...
              <div className="space-y-3">
                ...
                <Button onClick={() => upsert.mutate()} disabled={!form.name.trim()} className="w-full">{editId ? "Update" : "Add"} Vendor</Button>
              </div>
            </DialogContent>
          </Dialog>
        </div>

        <div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" /><Input placeholder="Search vendors..." value={search} onChange={e => setSearch(e.target.value)} className="pl-9 max-w-md" /></div>
        ...
        <Card>
          <CardContent className="pt-4">
            {isLoading ? <div className="py-8 flex justify-center"><Loader2 className="w-6 h-6 animate-spin text-primary" /></div> : filtered.length > 0 ? (
              ...
              <div className="py-12 text-center"><Truck className="w-12 h-12 text-muted-foreground mx-auto mb-4 opacity-20" /><h3 className="text-lg font-medium mb-1">No Vendors</h3><p className="text-muted-foreground">Add your first vendor to track supplier payments.</p></div>
            )}
          </CardContent>
        </Card>
      </div>
