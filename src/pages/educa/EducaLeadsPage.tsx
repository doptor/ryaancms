...
import { Plus, TrendingUp, Search, Pencil, Trash2, Loader2, UserPlus } from "lucide-react";
...
export default function EducaLeadsPage() {
  ...
  const filtered = (leads ?? []).filter(l => { const q = search.toLowerCase(); return !q || l.name.toLowerCase().includes(q) || (l.email || "").toLowerCase().includes(q) || (l.phone || "").includes(q); });
    ...
    <DashboardLayout>
      <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div><h1 className="text-2xl font-bold flex items-center gap-2"><TrendingUp className="w-6 h-6 text-primary" /> Leads</h1><p className="text-muted-foreground">CRM lead management</p></div>
