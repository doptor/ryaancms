...
import { Plus, Plane, Search, Pencil, Trash2, Loader2 } from "lucide-react";
...
export default function EducaVisaPage() {
  ...
  const filtered = (visas ?? []).filter(v => { const q = search.toLowerCase(); return !q || (v as any).educa_students?.name?.toLowerCase().includes(q) || (v.country || "").toLowerCase().includes(q) || (v.reference_number || "").toLowerCase().includes(q); });
    ...
    <DashboardLayout>
      <div className="p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div><h1 className="text-2xl font-bold flex items-center gap-2"><Plane className="w-6 h-6 text-primary" /> Visa Processing</h1><p className="text-muted-foreground">Track visa applications</p></div>
