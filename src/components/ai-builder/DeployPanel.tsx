...
export function DeployPanel({ config, sql, onExportJSON, onExportSQL }: DeployPanelProps) {
  ...
  const handleDownloadZip = async () => {
    // Create a zip-like multi-file download
    const files = [
      { name: "config.json", content: JSON.stringify({ _meta: { generator: "RyaanCMS AI Builder", version: "1.0.0", generated_at: new Date().toISOString() }, config }, null, 2), type: "application/json" },
      { name: "schema.sql", content: sql || "-- No schema generated", type: "text/sql" },
      { name: "README.md", content: `# ${config.title}\n\n${config.description}\n\nGenerated by RyaanCMS AI Builder.\n\n## Files\n\n- \`config.json\` — Application configuration\n- \`schema.sql\` — Database migration\n- \`setup.sh\` — Quick setup script\n\n## Setup\n\n\`\`\`bash\nchmod +x setup.sh && ./setup.sh\n\`\`\``, type: "text/markdown" },
      { name: "setup.sh", content: `#!/bin/bash\necho "Setting up ${config.title}..."\nnpm init -y\nnpm install\necho "Applying database schema..."\n# psql -f schema.sql\necho "Done! Run 'npm run dev' to start."`, type: "text/x-shellscript" },
    ];
    ...
    for (const file of files) {
      ...
      a.href = url;
      a.download = `${config.title?.toLowerCase().replace(/\s+/g, "-") || "project"}-${file.name}`;
      a.click();
      URL.revokeObjectURL(url);
      await new Promise((r) => setTimeout(r, 300));
    }
    toast({ title: "Downloaded!", description: "All project files downloaded." });
  };
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              ...
              <Button onClick={handleDownloadZip} className="w-full gap-2" size="sm">
                <FolderDown className="w-3.5 h-3.5" /> Download All (config + SQL + README + setup.sh)
              </Button>
            </div>
          </div>
        )}
